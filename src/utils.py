...
def sample_comments_evenly(comments: List[Dict[str, Any]], target_count: int) -> List[Dict[str, Any]]:
    ...
    if total_deficit > 0:
        logger.debug(f"Total deficit: {total_deficit}, begin supplementing from remaining segments")
        ...
        if segments_with_remaining:
            ...
            supplemental_plan: List[Tuple[int, List[Dict[str, Any]], int, int]] = []
            allocated = 0

            for seg_idx, segment, remaining in segments_with_remaining:
                base_quota = 0
                if total_remaining > 0 and remaining > 0:
                    base_quota = int(total_deficit * remaining / total_remaining)
                base_quota = max(0, min(remaining, base_quota))
                supplemental_plan.append((seg_idx, segment, remaining, base_quota))
                allocated += base_quota

            leftover = total_deficit - allocated
            if leftover > 0:
                supplemental_plan.sort(key=lambda item: item[2] - item[3], reverse=True)
                for idx in range(len(supplemental_plan)):
                    if leftover <= 0:
                        break
                    seg_idx, segment, remaining, base_quota = supplemental_plan[idx]
                    available_slots = remaining - base_quota
                    if available_slots <= 0:
                        continue
                    take = min(available_slots, leftover)
                    supplemental_plan[idx] = (seg_idx, segment, remaining, base_quota + take)
                    leftover -= take

            for seg_idx, segment, remaining, quota in supplemental_plan:
                if quota <= 0:
                    continue

                already_sampled_ids = {id(c) for c in sampled_comments if c in segment}
                available = [c for c in segment if id(c) not in already_sampled_ids]

                if not available:
                    continue

                actual_quota = min(quota, len(available))
                if actual_quota == len(available):
                    supplement = available
                else:
                    step = len(available) / actual_quota
                    supplement = [available[int(j * step)] for j in range(actual_quota)]

                sampled_comments.extend(supplement)
                logger.debug(f"Supplemented {actual_quota} comments from segment {seg_idx}")
